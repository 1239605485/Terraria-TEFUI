...
/* Do not store the launcher as a percentage of the value exposed while
   DrawVirtualControls is running: that value changes with the mobile stick
   and made the UI appear only after movement.  These are stable UI-space
   coordinates; both the launcher and menu derive from this same origin. */
static float g_launcher_x = 520.0f;
static float g_launcher_y = 180.0f;
...
static void draw_menu(void) {
    ...
    float launcher_x = g_launcher_x;
    float launcher_y = g_launcher_y;
        ...
        for (int i = 0; i < cursor_count; ++i) {
            ...
            if (sample.down) {
                ...
                if (map_cursor_position(&sample, g_cursor_transform,
                    ...
                    g_launcher_x = launcher_x;
                    g_launcher_y = launcher_y;
                }
            } else {
    ...
    /* Use the launcher's stable coordinate system rather than a second
       screen-centred origin.  This keeps the menu attached to the launcher. */
    const float menu_x = clampf_local(g_launcher_x, menu_half_width,
